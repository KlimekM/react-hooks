...
I enjoyed this lesson quite a bit, specifically the portions where we refactored
the component to use a status and then continued to refactor that to store the
entire state in an object. The ErrorBoundary bits were also great and it was fun
to see the use of the key trick passed to the ErrorBoundary to re-render that
portion of the tree.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=06%3A%20useEffect%3A%20HTTP%20requests&em=mklimek15%40gmail.com).
